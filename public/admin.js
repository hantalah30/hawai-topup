...
async function login() {
  ...
  if (!passInput || !btn) {
    console.error("Elemen login tidak ditemukan di HTML");
    return;
  }
  ...
  try {
    ...
    if (data.success) {
      ...
      alert("Password Salah! Cek console log Vercel untuk password asli.");
    }
  } catch (e) {
    console.error(e);
  ...
  data.forEach((p) => {
    ...
    tbody.innerHTML += `
            <tr class="${p.is_active ? "" : "table-light text-muted"}">
                ...
                <td><input type="number" class="form-control form-control-sm" style="width:80px" value="${markup}" onchange="updateMarkup(${realIndex}, this.value)"></td>
...
function updateMarkup(idx, val) {
  if (db.products[idx]) {
    db.products[idx].markup = parseInt(val) || 0;
    db.products[idx].price_sell =
      (db.products[idx].price_modal || 0) + db.products[idx].markup;
...
// [PENTING] GLOBAL EXPORT - Pastikan ini ada di paling bawah
window.login = login;
