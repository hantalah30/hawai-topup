...
// --- VOUCHER MANAGEMENT (LocalStorage) ---
const VOUCHER_DB_KEY = 'HAWAI_PROMO_DB';
...
function loadVouchers() {
    ...
    const raw = localStorage.getItem(VOUCHER_DB_KEY);
    let data;
    if (!raw) {
        data = { "HAWAI": { value: 0.1, type: "percent" } }; // Seed logic
        localStorage.setItem(VOUCHER_DB_KEY, JSON.stringify(data));
    } else {
        data = JSON.parse(raw);
    }

    if (Object.keys(data).length === 0) {
        tbody.innerHTML = '<tr><td colspan="4" class="text-center">Belum ada voucher.</td></tr>';
        return;
    }
    ...
    Object.keys(data).forEach(code => {
        ...
        let val = typeof item === 'number' ? item : item.value;
        let type = typeof item === 'number' ? (val < 1 ? 'percent' : 'flat') : item.type;

        let displayVal = type === 'percent'
            ? `${Math.round(val * 100)}%`
        ...
        <tr>
            ...
            <td>${displayVal}</td>
            <td>
                <button class="btn btn-sm btn-outline-danger" onclick="delVoucher('${code}')">
...
function addVoucher() {
    const elCode = document.getElementById('vCode');
    const elType = document.getElementById('vType');
    const elVal = document.getElementById('vVal');

    const code = elCode.value.toUpperCase().replace(/\s/g, '');
    const type = elType.value;
    const val = parseFloat(elVal.value);

    if (!code || isNaN(val)) return alert("Isi kode dan nilai dengan benar!");
