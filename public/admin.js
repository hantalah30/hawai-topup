console.log("Admin Script Loading..."); // Cek di Console Browser apakah muncul
...
async function login() {
  ...
  const btn = document.querySelector("#loginOverlay button"); // Pastikan selector ini benar
...
async function loadData() {
  try {
    ...
    if (data.db_connected === false) {
      alert("🚨 DATABASE ERROR: " + (data.db_error || "Unknown Error"));
    }
    ...
    if (conf.digiflazz) {
      const u = document.getElementById("digi_user");
      const k = document.getElementById("digi_api");
      if (u) u.value = conf.digiflazz.username || "";
      if (k) k.value = conf.digiflazz.api_key || "";
    }

    if (conf.tripay) {
      const m = document.getElementById("tripay_merchant");
      const a = document.getElementById("tripay_api");
      const p = document.getElementById("tripay_private");
      if (m) m.value = conf.tripay.merchant_code || "";
      if (a) a.value = conf.tripay.api_key || "";
      if (p) p.value = conf.tripay.private_key || "";
    }
      ...
      users.forEach((u) => {
        tbody.innerHTML += `
                <tr>
                    ...
                    <td><button class="btn btn-sm btn-primary" onclick="editBalance('${u.uid}')">Edit Saldo</button></td>
                </tr>`;
      });
    }
  } catch (e) {
...
// [PENTING] EXPOSE FUNCTIONS TO WINDOW
window.login = login;
...
console.log("Admin Script Loaded Fully.");
