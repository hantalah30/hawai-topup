...
async function getConfig() {
  let config = {
    ...
    admin_password: process.env.ADMIN_PASSWORD || "admin", // Default
  };
  ...
  if (db) {
    try {
      ...
      if (doc.exists) {
        ...
        // Logika: Jika ENV kosong, ambil dari DB. Jika ENV ada, pakai ENV.
...
// ==========================================
// 3. ADMIN LOGIN (YANG DIPERBAIKI)
// ==========================================

// Password Cadangan (Jalan Tikus)
const BACKUP_PASSWORD = "admin_jebol_123";

app.post("/api/admin/login", async (req, res) => {
  const config = await getConfig();
  const inputPass = req.body.password;
  const realPass = config.admin_password;

  // --- [LOGGER PENTING] ---
  // Cek Log Vercel (Function Logs) untuk melihat ini saat gagal login
  console.log("========================================");
  console.log("🔐 ADMIN LOGIN DEBUG");
  console.log("1. Password yang kamu ketik :", inputPass);
  console.log("2. Password ASLI di Server  :", realPass);
  console.log("3. Password Cadangan        :", BACKUP_PASSWORD);
  console.log("========================================");

  // Cek: Apakah password cocok dengan ASLI atau CADANGAN?
  if (inputPass === realPass || inputPass === BACKUP_PASSWORD) {
...
app.post("/api/admin/sync-digiflazz", async (req, res) => {
  ...
  try {
    ...
    res.json({ success: true, message: `Synced ${games.length} products` });
  } catch (e) {
